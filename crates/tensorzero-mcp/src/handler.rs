...
async fn handle_launch_optimization(
    ...
    .await
    .map_err(|e| {
        McpError::internal_error(format!("Failed to launch optimization: {e}"), None)
    })?;
    ...
    loop {
        ...
        .await
        .map_err(|e| {
            McpError::internal_error(format!("Failed to poll optimization: {e}"), None)
        })?;
...
async fn handle_launch_gepa(
    ...
    let spawn_result = spawn_client
        ...
        .await
        .map_err(|e| {
            McpError::internal_error(format!("Failed to spawn GEPA task: {e}"), None)
        })?;
